...
const styleListForChapterColor = chapterToColor.flatMap(chapter => [
  {
    ...
    style: {
      'color' : changeColor(chapter.color, statusVarColors.unavailable*0.95),
...
const graphStyle = [
    {
      ...
      style: {
        ...
        'width': `${defaultNodeWidth}px`,
        'height': `${defaultNodeHeight}px`,
        'border-width': '4px',
        'overlay-opacity': 0,
        ...
        'text-halign': 'center',
        'text-wrap': 'wrap',
        'text-max-width': `${defaultNodeWidth-20}px`,
        'line-height': '1.2',
        'color': '#333',
    ...
    },
    {
      ...
      style :{  
        'width': `${defaultNodeWidth*1.4}px`,
        'height': `${defaultNodeHeight*1.4}px`,
        'font-size': 'mapData(fontSize, 0, 100, 0, 150)',
        'border-width' : '8px',
      }
    },
    ...
    {
      selector : 'edge.vertical-layout',
      style:{
        'taxi-direction': 'vertical',
      }
    },
    {
      selector : 'edge.horizontal-layout',
      style:{
...
function getNodePositionsData() {
  return new Promise((resolve)=>{
    fetch('/data/node_positions.json')
    .then(res => res.json())
    .then((data) => {
      resolve(data);
    }).catch(err => console.error(err));
...
let verticalLayout = window.innerWidth<window.innerHeight
...
var storedNodePositions

async function loadEverything(){
  treeData = await getGraphData()
  nodePositions = await getNodePositionsData()
  storedNodePositions = JSON.parse(localStorage.getItem("storedNodePositions")) || nodePositions;
  progress = JSON.parse(localStorage.getItem("mathProgress")) || {} ;
  questionProgress = JSON.parse(localStorage.getItem("questionProgress")) || treeData.nodes.reduce(function(obj,n){
    if (n.data.quiz){
    obj[n.data.id] = {
      bestScore : 0,
      quizlength : n.data.quiz.length,
      validated: false,
    }}
    return obj;
  },{})
...
function setColorsAndLayout() {
  ...
  treeData.edges.forEach(e => {
    ...
    if (verticalLayout){
      e.classes.push('vertical-layout')
    } else {
      e.classes.push('horizontal-layout')
    }
  })
}
...
var klayOptions = {
  ...
  klay: {
    ...
    aspectRatio: verticalLayout? 2/3 : 1.5, // The aimed aspect ratio of the drawing, that is the quotient of width by height
    ...
    direction: verticalLayout?'DOWN':'RIGHT', // Overall direction of edges: horizontal (right / left) or vertical (down / up)
...
function updateStars(node){
  if (node.data('quiz') && questionProgress[node.id()].quizlength > 0){
    const stars = "⭐".repeat(Math.max(3-questionProgress[node.id()].quizlength+questionProgress[node.id()].bestScore,0))
...
function updateAllPositions(){
  verticalLayout = window.innerWidth<window.innerHeight
  if (verticalLayout){
    cy.edges().removeClass('horizontal-layout').addClass('vertical-layout')
  }else{
    cy.edges().removeClass('vertical-layout').addClass('horizontal-layout')
  }
  layout = cy.layout({
    name: 'preset',
    positions: storedNodePositions,
...
async function loadEventHandler(){
  ...
  if ((focusedOn === node) && isAccessible(node.id())){
...
function focusDirectNeighbors(node) {
 ...
 // Viewport size
  ...
  const nbColPredec = Math.floor((predecessors.length-1)/maxItemNumber)+1
  const itemPerColPredec = Math.floor((predecessors.length-1)/nbColPredec)+1
  const nbColSucc = Math.floor((successors.length-1)/maxItemNumber)+1
  const itemPerColSucc = Math.floor((successors.length-1)/nbColSucc)+1

  const maxLength = Math.max(itemPerColSucc, itemPerColPredec)

  let spacingX, spacingY;
  if (vw>vh){
    spacingY = defaultNodeHeight*1.1
    var totalHeight = spacingY*maxLength
    spacingX = Math.max(totalHeight/vh/2*vw,defaultNodeWidth*1.6)
  } else {
    spacingX = defaultNodeWidth*1.1
    var totalWidth = spacingX*maxLength
    spacingY = Math.max(totalWidth/vw/2*vh,defaultNodeHeight*1.6)
  }
  ...
  predecessors.forEach((n, i) => {
    positions[n.id()] = {
      x: centerX - spacingX - spacingX/1.7*(Math.floor(i/itemPerColPredec)),
      y: centerY + spacingY*(i % itemPerColPredec) + spacingY/2*(Math.floor(i/itemPerColPredec)) - totalHeightPred / (2*nbColPredec)
    };
  });
  successors.forEach((n, i) => {
    positions[n.id()] = {
      x: centerX + spacingX + spacingX/1.7*(Math.floor(i/itemPerColSucc)),
      y: centerY + spacingY*(i % itemPerColSucc) + spacingY/2*(Math.floor(i/itemPerColSucc)) - totalHeightSucc / (2*nbColSucc)
    };
  });
  subgraph.edges().addClass('horizontal-layout').removeClass('vertical-layout')
} else {
  predecessors.forEach((n, i) => {
    positions[n.id()] = { 
      x: centerX + spacingX*(i % itemPerColPredec) + spacingX/2*(Math.floor(i/itemPerColPredec)) - totalWidthPred / (2*nbColPredec),
      y: centerY - spacingY - spacingY/1.7*(Math.floor(i/itemPerColPredec)),
    };
  });
  successors.forEach((n, i) => {
    positions[n.id()] = { 
      x: centerX + spacingX*(i % itemPerColSucc) + spacingX/2*(Math.floor(i/itemPerColSucc)) - totalWidthSucc / (2*nbColSucc),
      y: centerY + spacingY + spacingY/1.7*(Math.floor(i/itemPerColSucc)),
    };
  });
  subgraph.edges().addClass('vertical-layout').removeClass('horizontal-layout')
}
